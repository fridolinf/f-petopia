...
class TabPetGrooming extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	showModal1Grooming = () => {
		this.setState({ isModalTambahPetG: true });
	};

	showEditGrooming = (id) => {
		this.setState({ isModalEditGrooming: true, id: id });
		this.props.getDetailProducts(id);
	};

	showDetailGrooming = (id) => {
		this.setState({
			isModalDetailGrooming: true,
		});
		this.props.getDetailProducts(id);
	};
	...
	onSubmitHandler = (value) => {
		...
		this.setState({ isModalTambahPetG: false });
	...
	onSubmitHandlerEdit = (value) => {
		...
		this.setState({ isModalEditGrooming: false });
				...
				render: (text, record) => (
					<Space size='small' direction='vertical'>
						...
						<button
							...
							onClick={(e) => {
								e.stopPropagation();
								this.showEditGrooming(record.id);
							}}
						>
							Edit
						</button>
						<button
							...
							onClick={(e) => {
								e.stopPropagation();
								this.showDetailGrooming(record.id);
							}}
						>
							Detail
						</button>
		...
		return (
			<div>
				...
				<div style={{ marginRight: '10rem' }}>
					<Button className='btn-info' onClick={this.showModal1Grooming}>
						Tambah Produk
					</Button>
				</div>

				<h4 className='my-3'>Data Produk Pet Grooming</h4>
				<Table columns={columnsPetGrooming} dataSource={this.props.data} />

				{/* Modal Pet Grooming  */}
				<Modal
					title='Tambah Produk'
					visible={isModalTambahPetG}
					footer={null}
					width={750}
					closable={false}
				>
